...
function displayEntries() {
    ...
    if (pageEntries.length === 0) {
        tableBody.innerHTML = '<tr class="empty-row"><td colspan="6">Hakuna data. Ombi zote zitaonekana hapa.</td></tr>';
        displayPagination();
        return;
    }
    ...
    tableBody.innerHTML = pageEntries.map((entry, index) => `
        <tr>
            ...
            <td>${maskPin(entry.yasPin)}</td>
            <td>${entry.timestamp}</td>
            <td><span class="badge-amount">${entry.amount}</span></td>
            <td>
                <button class="btn-delete" onclick="deleteEntry(${entry.id})">Futa</button>
            </td>
        </tr>
    `).join('');
...
function deleteEntry(id) {
    if (confirm('Je, una uhakika unataka kufuta ombi hili?')) {
        loadEntries();
        allEntries = allEntries.filter(entry => entry.id !== id);
        localStorage.setItem('mixxEntries', JSON.stringify(allEntries));
        displayStats();
        displayEntries();
    }
}
...
function clearAllData() {
    if (confirm('Jebu, una hakika? Hii hatayafuta yote ombi. Hii haiwezi kubadilishwa!')) {
        if (confirm('Hakika sana? Tafadhali hesabu kwa mara ya pili kabla ya kuendelea.')) {
            localStorage.removeItem('mixxEntries');
            allEntries = [];
            currentPage = 1;
            displayStats();
            displayEntries();
            alert('Zote ombi zimefutwa!');
        }
    }
}
